...
适配时应使用 `formatVersion: 5`。JSON 顶层类型、枚举值、日期格式、图片路径和 ID 关联必须符合下文约定。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `formatVersion` | integer | 是 | 自定义适配文件应固定写 `5`；当前应用接受 `1..5` |
...
| `mood` | integer 或 null | 否 | `1..10` | 兼容旧版单选心情；版本 5 应同时提供 `moods` |
| `moods` | integer array | 否 | 默认回退到 `mood` | 多选心情 ID；元素须唯一且位于 `1..10` |
| `createdAt` | long | 否 | 默认 `0` | 创建时间，Epoch 毫秒 |
| `updatedAt` | long | 否 | 默认 `0` | 更新时间，Epoch 毫秒 |
...
对于版本 1–4 的历史备份，应用会继续读取单值 `mood`（旧值 `1..6`），并自动转换成多选列表。版本 5 恢复时以 `moods` 为准；`mood` 仅保留一个代表值，用于向旧格式兼容。
...
如果某类数据为空，仍需为版本 5 创建对应文件。最小数据内容如下。
...
- 相同日记 ID 的原图片和心情列表会分别被导入日记中的对应列表替换。
...
- [ ] `manifest.json.formatVersion` 等于 `5`。
...
- [ ] 年龄在 `0..80`，生日不晚于恢复当天。
- [ ] 日记的 `moods` 不含重复值，且每个心情 ID 都在 `1..10`。
...
- 版本 3：开始读取 `time.json`。
- 版本 4：增加生日字段。
- 版本 5：当前导出版本，增加日记多选心情数组 `moods`。

新适配器不应生成旧版本。旧版本规则只用于理解历史备份；使用版本 5 可以完整保留睡眠、时光与多选心情数据。
